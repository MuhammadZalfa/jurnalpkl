<x-layout title="Penilaian Bulanan 3 - Instructor">
    <x-sidebar-instructor />
    
    <div class="flex-1 flex flex-col">
        <x-header-instructor title="Penilaian Bulanan 3" />
        
        <main class="flex-1 p-4 md:p-6">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-800">Penilaian Akhir Siswa: {{ $assessment->student->name }}</h1>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <form action="{{ route('instructor.assessment3.update', $assessment->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Skor (1-100)</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <input type="number" name="{{ $field }}_score" min="0" max="100" 
                                            class="w-20 border rounded px-2 py-1 text-center"
                                            value="{{ $assessment->monthly3->{$field.'_score'} ?? 0 }}">
                                    </td>
                                    <td class="px-6 py-4">
                                        <input type="text" name="{{ $field }}_desc" class="w-full border rounded px-2 py-1" 
                                            value="{{ $assessment->monthly3->{$field.'_desc'} ?? '' }}">
                                    </td>
                                </tr>
                                @endforeach
                                ...
                                <!-- Entrepreneurship -->
                                <tr>
                                    <td class="px-6 py-4 font-medium text-center">3</td>
                                    <td class="px-6 py-4 font-medium">Entrepreneurship</td>
                                    <td class="px-6 py-4"></td>
                                    <td class="px-6 py-4"></td>
                                </tr>
                                @foreach($entrepreneurship as $skill => $field)
                                <tr>
                                    <td class="px-6 py-4"></td>
                                    <td class="px-6 py-4">{{ $skill }}</td>
                                    <td class="px-6 py-4 text-center">
                    ...
                    <div class="p-6 border-t">
                        ...
                        <textarea name="dudi_comments" rows="3" class="w-full border rounded-md p-2" 
                            placeholder="Masukkan komentar untuk siswa...">{{ old('dudi_comments', $assessment->monthly3->dudi_comments ?? '') }}</textarea>
                        
                        <div class="mt-4 flex items-center">
                            <input type="hidden" name="dudi_approved" value="0">
                            <input type="checkbox" name="dudi_approved" id="dudi_approved" 
                                value="1" {{ (old('dudi_approved', $assessment->monthly3->dudi_approved ?? false)) ? 'checked' : '' }}
                            ...
                            <label for="dudi_approved">Setujui penilaian ini</label>
                        </div>
                    </div>
