<x-layout title="Penilaian Bulanan 1 - Instructor">
    <x-sidebar-instructor />
    
    <div class="flex-1 flex flex-col">
        <x-header-instructor title="Penilaian Bulanan 1" />
        ...
        <main class="flex-1 p-4 md:p-6">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <form action="{{ route('instructor.assessment1.update', $assessment->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Bobot (1-100)</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <!-- Soft Skills -->
                                <tr>
                                    <td class="px-6 py-4 font-medium" rowspan="11">Soft Skills</td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <input type="number" name="{{ $field }}" 
                                            ...
                                            value="{{ old($field, $assessment->monthly1?->{$field} ?? 0) }}">
                                    </td>
                                    <td class="px-6 py-4">
                                        <input type="text" name="{{ $field }}_desc" 
                                            class="w-full border rounded px-2 py-1" 
                                            value="{{ old($field.'_desc', $assessment->monthly1?->{$field.'_desc'} ?? '') }}">
                                    </td>
                                </tr>
                                @endforeach
                                <!-- Hard Skills -->
                                <tr>
                                    <td class="px-6 py-4 font-medium" rowspan="5">Hard Skills</td>
                                </tr>
                    ...
                    <div class="p-6 border-t">
                        ...
                        <textarea name="dudi_comments" rows="3" class="w-full border rounded-md p-2" 
                            placeholder="Masukkan komentar untuk siswa...">{{ old('dudi_comments', $assessment->monthly1->dudi_comments ?? '') }}</textarea>
                        
                        <div class="mt-4 flex items-center">
                            <input type="hidden" name="dudi_approved" value="0">
                            <input type="checkbox" name="dudi_approved" id="dudi_approved" 
                                value="1" {{ (old('dudi_approved', $assessment->monthly1->dudi_approved ?? false)) ? 'checked' : '' }}
